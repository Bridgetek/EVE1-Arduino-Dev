...
void EVE_CMD_ROMFONT(uint32_t font, uint32_t romfont)
{
    ...
    uint32_t fontsrc = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, PointerToFontGraphicsData));
    uint32_t width = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontWidthInPixels));
    uint32_t height = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontHeightInPixels));
    uint32_t format = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontBitmapFormat));
    uint32_t linestride = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontLineStride));
...
uint32_t font_getromptr(uint8_t fontnumber)
{
    ...
    {
        fontptr = fontroot + ((fontnumber - 16) * sizeof(Bridgetek_EVE1::GPU_FONT_HEADER));
    }
    else
    {
        fontptr = 0;
...
static void getfontinfocache(struct eve_font_cache *cache, uint8_t fontnumber, uint32_t fontptr, uint8_t first_character)
{
    ...
    // This determines the format of the font and how it is handled.
    {
        ...
        cache->width = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontWidthInPixels));
        cache->height = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontHeightInPixels));
        cache->format = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontBitmapFormat));
        cache->linestride = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontLineStride));
        // Get offset to glyphs.
        start_of_graphics = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, PointerToFontGraphicsData));
        // Load character widths and glyph pointers.
        int ch, w;
        for (ch = 0; ch < 128; ch += 4)
        {
            // Read character width as a 32 bit word.
            uint32_t width4 = eve.LIB_MemRead32(fontptr + (uint32_t)offsetof(Bridgetek_EVE1::GPU_FONT_HEADER, FontWidth[0]) + ch);
            for (w = 0; w < 4; w++)
            {
